...
def convert_to_binary(n):
    ...
    #"return True" ist eine dummy-Ausgabe
...
def generate_candidate():
    ...
    n = len(val) #n = Anzahl Gegenstände

    #Die Zufallszahlen werden in folgendem Bereich generiert. Die hier
    #angegebenen Grenzen sind falsch! Korrigieren Sie diese!
    minimum = 1
    maximum = 5
...
def is_solution(solution):
    ...
    #"return True" ist eine dummy-Ausgabe
    #und darf nicht so alleine stehen bleiben!)
    return True
def generate_population():
    ...
    population = []
    #Rufen Sie hier so lange die generate_candidate-Funktion auf, bis
    #eine gültige Lösung herauskommt. Fügen Sie diese Lösung der Liste
    #population hinzu. Wiederholen Sie das Ganze so oft, wie die
    #Variable population_size angibt, damit Sie eine Population von der
    #gewünschten Grösse erhalten.
...
## Test 2: Produziert generate_candidate Lösungskandidaten im richtigen Format?
...
## Test 4: Erkennt is_solution, ob ein Lösungskandidat gültig ist?
...
## Test 5: Erzeugt generate_population eine Liste gülter Lösungen der korrekten Länge?
## Die Ausgabe sollte so aussehen, wobei die erste Zeile
## jedes Mal etwas anders aussieht (aber immer aus 10 zu-
...
2) Population der Fitness nach sortieren (absteigend), gegebenenfalls
dezimieren, und die aktuell beste Lösung sowie deren Wert ausgeben auf der
Konsole.
3) So viele Paarungen (breed) durchführen wie in der Variablen pairing_rate
angegeben. Die jeweils dabei entstehenden Kinder zur Population hinzufügen.
4) Wiederhole Schritte 2 und 3 so häufig, bis genügend Generationen entstanden
sind (gemäss der Variablen generations).
"""
